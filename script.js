...
const p7Groups = [
  ...
  { type: "schedule", passage: "Schedule: New Hire Orientation, July 3. All employees starting this month should attend the in-person orientation program in Conference Room B. Please bring a government-issued photo ID, your signed employment forms, and a laptop if one has already been assigned to you. Check-in begins at 09:00. At 09:30, HR will lead a welcome session covering company policies, benefits enrollment, and workplace guidelines. At 10:30, the IT team will help participants complete account setup, password registration, and e-mail access. At 11:15, an office tour will introduce meeting rooms, emergency exits, supply areas, and department locations. At 12:00, new hires will have lunch with team leads to discuss first-week expectations and reporting procedures.", items: [
...
function renderAnswerFeedback(qItem, answerState) {
  ...
  if (answerState.legacy) {
    return `<div class='feedback answered-summary'><strong>此題已作答</strong><br>我的答案：舊版紀錄未保存作答選項<br>正確答案：${formatAnswerWithLabel(qItem, qItem.answer)}<br>解析：${esc(qItem.explanation)}<br>中文翻譯：${esc(qItem.questionTranslation || qItem.translation || "")}</div>`;
  }
  const lines = [
