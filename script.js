...
function renderQuestionCard(q, category){
  if (q.options && q.options.length) {
    return `<div class="card"><h3>${q.part || q.type}</h3><p>${q.question}</p>${q.audioUrl!==undefined?`<small class="muted">audioUrl: ${q.audioUrl || '(待補 mp3)'}</small>`:""}<div>${q.options.map(op=>`<button class='option-btn' data-id='${q.id}' data-ans='${op.replace(/'/g,"&#39;")}' >${op}</button>`).join("")}</div><div id='fb-${q.id}'></div></div>`;
  }
